...
describe("wrapRequestCallback", () => {
  describe("request acceptance", () => {
    context("SHIORI/3.x", () => {
      it("causes OK", async () => assert.strictEqual(
    ...
    context("SHIORI/2.x", () => {
      it("causes Bad Request", async () => assert.strictEqual(
  ...
  describe("defaultHeaders", function() {
    context("no default headers", function() {
      it("is same", async () => assert.deepStrictEqual(
        {Value: "1"},
    ...
    context("with default headers", function() {
      context("no same headers in return value", () => {
        ...
        it("appended", async () => assert.deepStrictEqual(
          {Value: "1", To: "sakura"},
      ...
      context("same headers exist in return value", () => {
        ...
        it("is not overwrited", async () => assert.deepStrictEqual(
          {Value: "1"},
          (await wrapRequestCallback(() => 1, defaultHeaders)(request3)).headers.header),
        );
      });
...
describe("wrapRequestStringCallback", () => {
  ...
  it("same response as wrapRequestCallback", async () => assert.strictEqual(
